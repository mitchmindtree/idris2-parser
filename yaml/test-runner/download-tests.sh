...
for test in $TESTS; do
  count=$((count + 1))
  printf "\r[$count/$total] Downloading $test..."
  mkdir -p "$OUT_DIR/$test"

  # Download in.yaml (required)
  curl -sf "$BASE_URL/$test/in.yaml" -o "$OUT_DIR/$test/in.yaml" 2>/dev/null || true

  # Download error file if it exists (indicates expected failure)
  curl -sf "$BASE_URL/$test/error" -o "$OUT_DIR/$test/error" 2>/dev/null || rm -f "$OUT_DIR/$test/error"

  # Download test name
  curl -sf "$BASE_URL/$test/===" -o "$OUT_DIR/$test/===" 2>/dev/null || true

  # Download expected event output
  curl -sf "$BASE_URL/$test/test.event" -o "$OUT_DIR/$test/test.event" 2>/dev/null || true
done

echo ""
echo "Done! Downloaded $total tests to $OUT_DIR"
